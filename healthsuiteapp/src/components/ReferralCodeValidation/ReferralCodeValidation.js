...
const ReferralCodeValidation = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (unknownReferralCode) {
      setValidationMessage(
        `Please contact admin from the home page to retrieve your refcode`
      );
    } else {
      setValidationMessage(
        `The code you have entered is for ${data.participantName}. Please confirm that your code has been entered correctly:`
      );
    }
    setOpenDialog(true);
  };
      ...
      <Container maxWidth="sm" className="referral-code-validation-container">
        ...
        <form onSubmit={handleSubmit}>
          <FormControlLabel
            control={
              <Checkbox
                checked={unknownReferralCode}
                onChange={handleUnknownReferralCodeChange}
              />
            }
